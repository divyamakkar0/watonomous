...
MapMemoryNode::MapMemoryNode(float dt) : Node("map_memory"), map_memory_(robot::MapMemoryCore(this->get_logger())) {
  ...
  this->last_pos = std::make_pair(0.0, 0.0);
  this->dt = dt; 
  mapPub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("/map", 10);

  timer_ = this->create_wall_timer(
    std::chrono::seconds(0.5), std::bind(&MapMemoryNode::publishMap, this));
}
...
void MapMemoryNode::readCostmap(const nav_msgs::msg::OccupancyGrid msg){
  ...
  std::vector<int>occupancyGrid (length * length);
  occupancyGrid = msg->occupancy_grid;
